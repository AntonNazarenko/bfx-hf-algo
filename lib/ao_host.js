...
const {
  subscribe, unsubscribe, findChannelId, Manager
...
class AOHost extends AsyncEventEmitter {
  ...
  constructor (args = {}) {
    ...
    const {
      apiKey, apiSecret, agent, wsURL, restURL, aos, db, dms = 4,
    } = args
    ...
    this.onWSReopen = this.onWSReopen.bind(this)
    this.onSocketUpdate = this.onSocketUpdate.bind(this)
    this.onServerRestart = this.onServerRestart.bind(this)
    ...
    this.m.on('ws2:event:info-server-restart', this.onServerRestart)
    this.m.on('ws2:reopen', this.onWSReopen)
    ...
    bindWS2Bus(this)
    registerAOUIs(this).catch((err) => {
      debug('failed to register AO UIs: %s', err.message)
    })
  ...
  onWSReopen () {
    ...
    this.reloadAllAOs().catch((err) => {
      debug('error reloading all AOs: %s', err.message)
    })
  }
